...
export const postMethod = async (backend_url, body, headers, error, returnData) => {
    ...
    .then(async (res) => {
        try {
            let data = await res.json();
            if (res.status === 204) { // HTTP 204 No Content success status (no matches)
                return {};
            }
            if (res.status === 200) {
                return returnData ? data : true;
            } else {
                console.error(error + ': ' + (typeof data.msg === 'string' ? data.msg : data.sqlMessage ?? data.msg?.sqlMessage))
                return false;
            }
        }
        catch (err) {
